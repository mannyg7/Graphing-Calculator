...
class CalculatorViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let graphViewController = destinationViewController as? GraphingViewController,
            let identifier = segue.identifier,
            identifier == "Graphing View" {
            let result = brain.evaluate(using: variables)
            if result.isPending {
                return
            }
            graphViewController.graphingFunction = {
                [weak weakSelf = self] x in weakSelf?.variables["M"] = x
                return weakSelf?.brain.evaluate(using: weakSelf?.variables).result
            }
            graphViewController.navigationItem.title = "y = " + result.description
